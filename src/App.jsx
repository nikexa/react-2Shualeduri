...
function App() {
    ...
    if (deleteReply) {
      SetComments(
        comments.map((comment) => {
          ...
          return {
            ...comment,
            Reply: comment.Reply.filter((reply) => reply.id !== deleteId),
          };
        })
      );
    } else {
  ...
  function handleEdit(editid, IsEdited = false) {
    if (IsEdited) {
      SetComments(
        comments.map((comment) => {
          return {
            ...
            Reply: comment.Reply.map((reply) =>
              reply.id === editid
                ? { ...reply, text: editedText || reply.text }
                : reply
            ),
          };
        })
      ...
      SetComments(
        comments.map((comment) =>
          comment.id === editid
            ? { ...comment, text: editedText || comment.text }
            : comment
        )
      );
    }
  ...
  return (
    <div
      className={`w-full h-screen ${OpenModal ? "bg-[#00000080]" : "bg-white"}`}
    >
            ...
            return (
              <>
                  ...
                  <div className="flex flex-col w-[40px] h-[100px] items-center place-content-around mt-[0px] ml-[50px] max-md:ml-1.5 max-md:flex-row max-md:justify-between max-md:w-[330px]">
                    ...
                    <div className="mt-[7px] gap-[25px] hidden max-md:flex">
                      <button
                        onClick={() => {
                          SetOpenModal(true);
                          setDelteId(coment.id);
                          setDelteReply(false);
                        }}
                        className="flex items-center gap-[10px] cursor-pointer text-[#ED6368] hover:text-[#FFB8BB] "
                      >
                        <i className="fa-solid fa-trash-can"></i>
                      </button>
                      <button
                        onClick={() => SetEdit(coment.id)}
                  ...
                  <div key={coment.id} className="flex flex-col ">
                    <div className="flex w-[620px] place-content-between max-md:w-[343px]">
                      ...
                      <div className="mt-[7px] flex gap-[25px] max-md:hidden">
                        ...
                        >
                          <i className="fa-solid fa-trash-can"></i>
                          <p className="font-[Rubik] font-normal text-[16px] ">
                            Delete
                          </p>
                        </button>
                        <button
                    ...
                    {edit === coment.id ? (
                      <div ref={editRef} className="mt-[20px] flex gap-[15px]">
                        <textarea
                          ...
                          className="w-[500px] h-[124px] border border-[#5357B6] resize-none outline-none rounded-[8px] pl-[20px] pr-[20px] pt-[5px] font-[Rubik] font-normal text-[16px] text-[#334253] max-md:w-full"
                        ></textarea>
                        <button
                          onClick={() => handleEdit(coment.id)}
                          className="w-[104px] h-[48px] rounded-[8px] bg-[#5357B6] text-white text-[16px] font-[Rubik] font-normal cursor-pointer max-md:text-[14px]"
                        >
                          UPDATE
                        </button>
                      </div>
                ...
                {reply === coment.id && (
                  <div
                    ref={replyRef}
                    className="w-[730px] flex justify-end items-start gap-[15px] max-md:w-[325px]"
                  >
                    <img src={pfp} alt="" />
                    <textarea
                      onChange={(e) => setReplyText(e.target.value)}
                      value={replyText}
                      className="border border-[#5357B6] outline-none w-[506px] h-[96px] rounded-[8px] pl-[20px] pr-[20px] pt-[5px] resize-none font-[Rubik] font-normal text-[16px] text-[#334253] max-md:w-full"
                    />
                    <button
                      onClick={() => HandleSubmitReply(coment.id)}
                      className="w-[105px] h-[48px] rounded-[8px] bg-[#5357B6] hover:bg-[#C5C6EF] font-[Rubik] font-normal text-[16px] text-white cursor-pointer max-md:text-[14px]"
                    >
                      REPLY
                    </button>
                  </div>
                  ...
                  coment.Reply.map((Eachreply) => (
                    <div className="flex justify-between max-md:w-[330px] max-md:ml-[5px]">
                      ...
                      >
                        <div className="flex flex-col w-[40px] h-[100px] items-center place-content-around mt-[0px] ml-[0px] max-md:ml-1.5 max-md:flex-row max-md:justify-between max-md:w-[330px]">
                          <div className="flex flex-col w-[40px] h-[100px] items-center place-content-around mt-[5px] mr-[20px] max-md:flex-row max-md:gap-[10px] max-md:h-[40px] max-md:w-[100px] max-md:ml-0 ">
                            ...
                            <p className="font-[Rubik] w-full font-normal text-[16px] text-[#5357B6]">
                              {Eachreply.count}
                            </p>
                            <button
                              className="w-full h-[15px]"
                          ...
                          <div className="hidden max-md:flex max-md:gap-[25px]">
                            ...
                            <button
                              onClick={() => SetEdit(Eachreply.id, true)}
                              className="flex items-center gap-[10px] cursor-pointer text-[#5357B6] hover:text-[#C5C6EF] "
                            >
                              <i className="fa-solid fa-pen"></i>
                            </button>
                        ...
                        >
                          ...
                          {edit === Eachreply.id ? (
                            ...
                            >
                              ...
                              ></textarea>
                              <button
                                onClick={() => handleEdit(Eachreply.id, true)}
                      ...
                      {OpenModal && (
                        <div
                          ref={deleteREf}
                          className="w-[400px] h-[252px] bg-white rounded-[8px] flex flex-col items-center justify-center gap-[20px] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]"
                        >
                          <p className="font-[Rubik] font-normal text-[24px] text-[#334253] w-[336px]">
                            Delete comment
                          </p>
                          <p className="font-[Rubik] font-normal text-[16px] text-[#334253] w-[336px]">
                          ...
                          <div className="flex gap-[20px]">
                            <button
                              ...
                              className="w-[161px] h-[48px] bg-[#67727E] text-white text-[16px] font-[Rubik] font-normal rounded-[8px]"
                            >
                              NO, CANCEL
                            </button>
                            <button
                              onClick={HandleDeleteComment}
                              className="w-[161px] h-[48px] bg-[#ED6368] text-white text-[16px] font-[Rubik] font-normal rounded-[8px]"
                            >
                              YES, DELETE
                            </button>
                          </div>
                ...
                {OpenModal && (
                  <div
                    ref={deleteREf}
                    className="w-[400px] h-[252px] bg-white rounded-[8px] flex flex-col items-center justify-center gap-[20px] absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] max-md:w-[343px] max-md:h-[224px]"
                  >
                    <p className="font-[Rubik] font-normal text-[24px] text-[#334253] w-[336px] max-md:text-[20px] max-md:w-[288px]">
                      Delete comment
                    </p>
                    <p className="font-[Rubik] font-normal text-[16px] text-[#334253] w-[336px] max-md:w-[288px]">
                    ...
                    <div className="flex gap-[20px]">
                      <button
                        onClick={() => SetOpenModal(false)}
                        className="w-[161px] h-[48px] bg-[#67727E] text-white text-[16px] font-[Rubik] font-normal rounded-[8px] max-md:w-[138px] max-md:h-[48px]"
                      >
                        NO, CANCEL
                      </button>
                      <button
                        onClick={HandleDeleteComment}
                        className="w-[161px] h-[48px] bg-[#ED6368] text-white text-[16px] font-[Rubik] font-normal rounded-[8px] max-md:w-[138px] max-md:h-[48px]"
                      >
                        YES, DELETE
                      </button>
                    </div>
